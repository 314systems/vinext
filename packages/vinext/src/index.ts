...
const VINEXT_VIRTUAL_LOAD_ID_RE = new RegExp(
  `^${escapeRegExp(VIRTUAL_PREFIX)}virtual:vinext-(?:server-entry|client-entry|rsc-entry|app-ssr-entry|app-browser-entry|app-capabilities|root-params|cache-adapters|image-adapters|google-fonts(?:\\?|$))`,
);
...
export default function vinext(options: VinextOptions = {}): PluginOption[] {
  ...
  const plugins: PluginOption[] = [
    ...
    dataUrlCssPlugin(),
    {
      ...
      load: {
        filter: { id: VINEXT_VIRTUAL_LOAD_ID_RE },
        async handler(id) {
          // Pages Router virtual modules
...
`;
    ...
    createInstrumentationClientTransformPlugin(() => instrumentationClientPath),
    {
      ...
      resolveId: {
        filter: { id: new RegExp(`^${escapeRegExp(VIRTUAL_INSTRUMENTATION_CLIENT)}$`) },
        handler(id) {
      ...
      load: {
        filter: { id: new RegExp(`^${escapeRegExp(RESOLVED_INSTRUMENTATION_CLIENT)}$`) },
        handler(id) {
    ...
    // instead of crashing with "does not provide an export named 'ViewTransition'".
    {
      ...
      load: {
        filter: { id: new RegExp(`^${String.fromCharCode(0)}virtual:vinext-react-canary$`) },
        handler(id) {
