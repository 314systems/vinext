...
| `reactCompiler`                                  | ✅  | `true` or `{ compilationMode, panicThreshold }`. Requires `babel-plugin-react-compiler`; with `@vitejs/plugin-react` v6, also install `@rolldown/plugin-babel`. Applied to client transforms only. Not applied when `vinext({ react: false })`. |
